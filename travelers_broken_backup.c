    ...
    for (int i = 0; i < travelerCount; i++) {
        ...
        if (pid == 0) {
            ...
            for (int j = 0; j < travelers[i].pathLength; j++) {
                Message msg;
                msg.travelerIndex = i;
                msg.currentNode = travelers[i].shortestPath[j];
                msg.pid = getpid();

                if (j == travelers[i].pathLength - 1) {
                    msg.nextNode = -1;
                    msg.finished = 1;
                    printf("[PID=%d] arrived at node %d | DESTINATION\n", getpid(), msg.currentNode);
                } else {
                    msg.nextNode = travelers[i].shortestPath[j + 1];
                    msg.finished = 0;
                    printf("[PID=%d] arrived at node %d | next node: %d\n", getpid(), msg.currentNode, msg.nextNode);
                }
    ...
    while (!WindowShouldClose()) {
        ...
        for (int i = 0; i < travelerCount; i++) {
            ...
            if (read(pipes[i][0], &msg, sizeof(Message)) > 0) {
                travelers[msg.travelerIndex].pid = msg.pid;

                if (msg.currentNode >= 0 && msg.currentNode < nodeCount) {
